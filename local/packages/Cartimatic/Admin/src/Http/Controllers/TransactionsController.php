...
use Cartimatic\Store\StoreBankAccount;
...
class TransactionsController extends Controller{
    ...
    public function index(){
        ...
        foreach ($transactions as $item) {
            ...
            if(!isset($last_month_sales->id)){
                continue;
            }

            $item->total_orders = $newTran = StoreTransaction::where('user_id',$item->user_id)
            ...
            $item->user_id = $userInfo->userable_id;
    ...
    public function withddrawalRequests(){
        ...
        $query = $query->where('witdrawal_type', '!=', 'affiliate');
    ...
    public function affiliateWithdrawalRequests(){
        ...
        $query = StoreWithdrawal::orderBy('id','DESC');
        $query = $query->where('witdrawal_type', 'affiliate');
        $query = $query->where('witdrawal_type', '!=', 'store');
        if(!empty($term)){
            $users = User::where('displayname','like',"%$term%")
                         ->lists('id','id')->toArray();
            if(!empty($users)){
        ...
        return view('Admin::transactions.storeAffiliateWithdrawlRequests',$data);
    }
