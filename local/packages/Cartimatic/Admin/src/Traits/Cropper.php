 ...
 * Date         : 03-May-16 3:54 PM
 * File Name    : Cropper.php
 */

namespace Cartimatic\Admin\Traits;
...
trait Cropper
{
        ...
        if($errorCode === UPLOAD_ERR_OK) {
          ...
          if ( function_exists( 'exif_imagetype' ) ){
            ...
            if($type) {
                ...
                if(!$this->folderExists($bannerType)) {
                    $this->createDirectory($bannerType);
                }
    ...
    private function crop($type, $quality=100) {
        ...
        if(!empty($src) && !empty($dst) && !empty($data)) {
          ...
          $size   = getimagesize($src);
            ...
            $dst_img_w = \Input::get('width');
            $dst_img_h = \Input::get('height');
            ...
            return $response = array(
               ...
               // 'result' => $this -> getResult($type)
                'result' => $this -> compress_image(asset('local/storage/app/'.$this -> getResult($type)), 'local/storage/app/'.$this -> getResult($type), $quality)
            );
        }
    }
  ...
  public function compress_image($source_url, $destination_url, $quality) {
    ...
    public function getResult($type) {
       // return !empty($this->data) ? url('photo/'.$type.'/'.$this->fileUrl) : '';
