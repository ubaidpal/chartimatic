...
use Cartimatic\Store\StoreOrderStatusLog;
...
class StoreAdminOrderRepository {
	...
	public function getAllOrdersCurrentUser($user_id=0) {
		return $allOrders = StoreOrder::where('seller_id', $user_id)->orderBy('id', 'DESC')->get();
	}
	public function paginateUserOrders($user_id, $status = null, $order_number = null){

		$query = StoreOrder::where('seller_id', $user_id)->orderBy('id', 'DESC');
	...
	public function searchMyOrders( $order_number, $product_name , $user_id=0)
	{
		 ...
		 $allOrders = \DB::table('store_orders')
				...
				->where('store_orders.seller_id', '=', $user_id)
