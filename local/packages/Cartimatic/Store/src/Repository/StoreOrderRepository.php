...
class StoreOrderRepository
{
     ...
     * @return int
     */
    public function getAllOrdersCurrentUser($user_id=0) {
        return $allOrders = StoreOrder::where('is_deleted', 0)->where('customer_id', $user_id)->with('delivery')->orderBy('id', 'DESC')->get();
    }

    public function userOrderToArray($user_id, $order_id = 0)
    {
        $orders =  StoreOrder::select(

          'a1.label as attr_1_label', 'a2.label as attr_2_label',

          'av1.value as attr_1_value', 'av2.value as attr_2_value',

          'k.master_attribute_1',

          'u.displayname as seller_name', 'u.username as seller_profile_link',

          'p.title as product_name', 'p.id as product_id',

          //'p_img.image_path as product_image_path',

          'store_orders.order_number', 'store_orders.id as my_order_id', 'store_orders.total_shiping_cost', 'store_orders.total_price as order_total_price', 'store_orders.total_quantity as order_total_quantity',

          'i.id as my_order_item_id', 'i.product_price as product_price', 'i.quantity as product_quantity'
        )

          ->join("users as u", 'u.id', '=', 'store_orders.seller_id' )
          ->join("store_order_items as i", 'i.order_id', '=', 'store_orders.id' )
          ->join("store_products as p", 'p.id', '=', 'i.product_id' )
          //->join("store_product_images as p_img", 'p_img.product_id', '=', 'i.product_id' )
          ->join("store_products_keeping as k", 'k.id', '=', 'i.product_keeping_id' )

          ->join("store_attributes as a1", 'a1.id', '=', 'k.master_attribute_1' )
          ->join("store_attributes as a2", 'a2.id', '=', 'k.master_attribute_2' )

          ->join("store_attribute_values as av1", 'av1.id', '=', 'k.master_attribute_1_value' )
          ->join("store_attribute_values as av2", 'av2.id', '=', 'k.master_attribute_2_value' )

          ->where('customer_id', $user_id)
          ->where('store_orders.id', $order_id)

          ->orderBy('store_orders.id', 'DESC')
          ->get()
          ->toArray();
        ...
        foreach ($orders as $order) {
            ...
            $order['seller_profile_link'] = url('store').'/'.$order['seller_profile_link'];

            $p_image = StoreProductImage::select('image_path as product_image_path')->where('product_id', $order['product_id'])->orderByRaw("RAND()")->get();

            $order['product_image_path']  = '';

            if(isset($p_image[0]->product_image_path)){
                $order['product_image_path']  = url('local/storage/app/').'/'.$p_image[0]->product_image_path;
            }


            $orderArr['order-'.$order['my_order_id']][] = $order;
        }

        return $orderArr;
    }
    public function userOrdersToArray($user_id, $status = null){
        ...
        if(!empty($status)){
            $orders =  StoreOrder::select(
              ...
              'u.id as brand_id', 'u.displayname as seller_name', 'u.username as seller_profile_link',

              'p.title as product_name', 'p.id as product_id',

              //'p_img.image_path as product_image_path',

              'store_orders.order_number', 'store_orders.created_at', 'store_orders.id as my_order_id', 'store_orders.total_shiping_cost', 'store_orders.total_price as order_total_price', 'store_orders.total_quantity as order_total_quantity', 'store_orders.status as order_status',
              ...
              ->where('store_orders.status', $status)
              ->where('customer_id', $user_id)
              ->orderBy('store_orders.id', 'DESC')
              ->get()
              ->toArray();
            ...
            foreach ($orders as $order) {
                ...
                $orderArr['order-'.$order['my_order_id']][] = $order;
                $orderArr['order-'.$order['my_order_id']]['order_info'] = ['order_number' => $order['order_number'],
                                                                           'order_id' => $order['my_order_id'],
                                                                           'brand_id' => $order['brand_id'],
                                                                           'seller_profile_link' => $order['seller_profile_link'],
                                                                           'seller_name' => $order['seller_name'],
                                                                           'total_shiping_cost' => $order['total_shiping_cost'],
                                                                           'order_total_quantity' => $order['order_total_quantity'],
                                                                           'created_at' => $order['created_at'],
                                                                           'sub_total' => $order['order_total_price'] - $order['total_shiping_cost'],
        ...
        foreach ($orders as $order) {
            ...
            $orderArr['order-'.$order['my_order_id']]['products'][] = $order;
            ...
            $orderArr['order-'.$order['my_order_id']]['order_info'] = ['order_number' => $order['order_number'],
             ...
             'order_status' => config("constants_brandstore.ORDER_STATUS_MESSAGE.".$order['order_status']),
            ];
        }
    ...
    public function paginateUserOrders($user_id,$status = null, $perPage=null){
        if($perPage < 1){
            $perPage = 10;
        }
        $query = StoreOrder::where('customer_id', $user_id)->orderBy('id', 'DESC');
        if($status != null){
    ...
    public function updateOrderStatus($order_id, $user_id)
    {
        return StoreOrder::where('customer_id', $user_id)->where('id', $order_id)
          ->update(['status' => \Config::get("constants_brandstore.ORDER_STATUS.ORDER_DELIVERED")]);
    }

    public function countRequestToReviseCurrentUser($user_id) {
        $productReviewCount = 0;

        $orderIds = StoreOrder::where('status', \Config::get("constants_brandstore.ORDER_STATUS.ORDER_DELIVERED"))->where('customer_id', $user_id)->lists('id');
        ...
        foreach($orderProductsIds as $orderProductsId){
            $review = StoreProductReview::where('product_id', $orderProductsId)->where('owner_id', $user_id)->first();
            if(isset($review->id)){
    ...
    public function paymentReceivedInfo($user_id=0) {
        return $allOrders = StoreOrderTransaction::where('user_id', $user_id)->get();
    }
    ...
    public function checkProductShippingCountry($product_ids, $country_id, $sub_total = 0) {
        ...
        if (!empty($data['totalShippingCost'])) {
            $data['totalShippingCost'] = $data['totalShippingCost'];
            $data['grand_total']       = $sub_total + $data['totalShippingCost'];
        } else {
    ...
    public function getProductShippingCost($product_id) {
        // Calculate shipping cost every time, whenever it is needed.
    ...
    public function getOrderTotalShippingCost($allOrderProductsIds, $country_id, $sellerBrandId = 0) {
        ...
        foreach ($allOrderProductsIds as $brand => $products) {
            foreach ($products as $orderProduct) {
                ...
                //if (isset($regionCostInfo->shipping_cost)) {
                    $totalShippingCost = $totalShippingCost + ($productInfo->shipping_cost * $orderProduct['quantity']);
                //}
            }
        }
    ...
    public function countOrdersStatusWise($customer_id)
    {
        ...
        foreach ($order_status as $key => $value){
            if($key == 'ORDER_AWAITING_PAYMENT' || $key == 'ORDER_PAYMENT_BEING_VERIFIED' || $key == 'ORDER_DELIVERED'){
                continue;
            }
    ...
    public function searchMyOrders($order_number, $product_name, $user_id=0) {
        $allOrders = \DB::table('store_orders')
            ...
            ->where('store_orders.customer_id', '=', $user_id)
    ...
    public function searchMyReviews($order_number, $product_name, $user_id=0) {
        $allOrders = \DB::table('store_orders')
            ->select('store_orders.id', 'order_number', 'store_orders.customer_id', 'store_orders.seller_id', 'delivery_address_id', 'payment_type', 'status', 'is_deleted', 'total_price', 'total_shiping_cost', 'total_discount', 'total_quantity', 'store_orders.created_at', 'approved_date', 'shiping_date', 'received_date')
            ->join('store_order_items', 'store_order_items.order_id', '=', 'store_orders.id')
            ->join('store_products', 'store_products.id', '=', 'store_order_items.product_id')
