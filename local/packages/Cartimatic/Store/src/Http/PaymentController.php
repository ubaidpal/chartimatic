...
// setup PayPal api context
    ...
    {
        $data['buy_it_now'] = (isset($request->buy_it_now) AND $request->buy_it_now != 'no') ? Hashids::decode($request->buy_it_now)[0]:'no';
        ...
        if ($method == 'paypal') {
            ...
            {
                return view("cart.paypalPayment", $data);
            }
            ...
            if(!empty($this->theme_id))
            {
                return view("cart.cardPayment", $data);
            }
            return view("Store::Cart.cardPayment", $data);

        } elseif ($method == 'cash-on-delivery') {
            $data['buy_it_now'] = (isset($request->buy_it_now) AND $request->buy_it_now != 'no') ? $request->buy_it_now:'no';

            return view("Store::Cart.cashOnDeliveryPayment", $data);
        }else {
            ...
            if(!empty($this->theme_id))
            {
                return view("cart.reviewOrder", $data)->with('payment_type_error', '1');;
            }
    ...
    public function makePayment(\Illuminate\Http\Request $request)
    {
        ...
        try {
            ...
            $orderUserInfo = getUserDetail( $address->user_id );
            ...
            $response = $world_pay->createOrder(array(
                ...
                'orderDescription' => 'Order from user:' . (isset($orderUserInfo->displayname)?$orderUserInfo->displayname:$inputNameWorldPay) . ' received.',
                'customerOrderCode' => ''
            ));

            if ($response['paymentStatus'] === 'SUCCESS') {

                $order_ids = $this->storeRepository->storeOrder($data['sellerBrandId'], (isset($orderUserInfo->id)?$orderUserInfo->id:''));

                foreach ($order_ids as $orderId) {
                    $order = StoreOrder::where('id',$orderId)->first();
                    $order_total = str_replace(',','',$order->total_price) - str_replace(',','',$order->total_discount);
                    $this->saveTransaction($response, $order->id, $order_total, (isset($orderUserInfo->id)?$orderUserInfo->id:''));
                    ...
                    foreach ($orderItems as $orderItem) {
                        ...
                        $productShippingCost   = getProductShippingCost($orderItem->product_id);
    ...
    public function cashOnDeliveryPayment(\Illuminate\Http\Request $request)
    {
        $data['buy_it_now'] = (isset($request->buy_it_now) AND $request->buy_it_now != 'no') ? Hashids::decode($request->buy_it_now)[0]:'no';
        if($request->sellerBrandId != "buy-all"){
            $data['sellerBrandIdEncoded'] = $request->sellerBrandId;
        ...
        $address_id = $this->storeRepository->getCartDeliverAddress();
        $address = $this->storeRepository->getDeliveryAddressByID($address_id);
        $cartItems = $this->storeRepository->getCartProducts($data['sellerBrandId'], $data['buy_it_now']);
        if(empty($cartItems)){
            return redirect()->to('store/cart/your-cart-is-empty');
        }
        ...
        $order_ids = $this->storeRepository->storeOrder($data['sellerBrandId'], (isset($orderUserInfo->id)?$orderUserInfo->id:''), 1, $data['buy_it_now']);
        ...
        foreach ($order_ids as $orderId) {
            ...
            $response['paymentResponse']['type'] = 'CashOnDelivery';
            $response['paymentStatus'] = 'PENDING';
            $response['orderCode'] = '';
            $response['amount'] = $order_total * 100;
            $response['currencyCode'] = '';

            $this->saveTransaction($response, $order->id, $order_total, (isset($orderUserInfo->id) ? $orderUserInfo->id : ''),3);
    ...
    protected function saveTransaction($respone, $order_id=0, $orderPrice, $userId,$gateway_id = 2)
    {
        ...
        $orderTransaction->gateway_id = $gateway_id;
        ...
        if($order_id != 0){
            $this->updateSoldProductQuantityAfterCheckout($order_id);
        }

    }

    public function updateSoldProductQuantityAfterCheckout($order_id=0)
    {
        ...
        $products = StoreOrderItems::where('order_id', $order_id)
          ->select(['id', 'order_id', 'quantity', 'product_id', 'product_keeping_id'])
          ->get();
        ...
        foreach ($products as $item) {
            ...
            $store_product_keeping_qty = StoreProductKeeping::select('quantity')->where('id', $item->product_keeping_id)
              ->first();

            if (isset($store_product->id)) {
                $store_productSold = (INT)$store_product->sold + (INT)$item->quantity;
                StoreProduct::where('id', $item->product_id)->update(['sold' => $store_productSold]);
            }

            if (isset($store_product_keeping_qty->quantity)) {
                $store_product_keeping_qtyQuantity = (INT)$store_product_keeping_qty->quantity - (INT)$item->quantity;
                StoreProductKeeping::where('id', $item->product_keeping_id)
                  ->update(['quantity' => $store_product_keeping_qtyQuantity]);
            }
        }
    }
