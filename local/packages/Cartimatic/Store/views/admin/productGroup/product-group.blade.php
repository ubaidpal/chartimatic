...
@section('content')
    ...
    <div class="col-md-12" style="background-color: #fff">
        ...
        <div class="col-md-6 col-md-offset-3">
            <div class="row">
                {!! Form::model(null , ['method' => 'POST', 'url' => "store/$store_id/admin/savedProductGroup","id" =>"productGroup"]) !!}
                ...
                <div class="form-group row">
                    ...
                    <div class="col-xs-10">
                        <textarea class="form-control" name="comments"
                                  placeholder="Type your comments">{{@$comments}}</textarea>
                    </div>
                </div>
                <input type="hidden" value="{{$store_id}}" id="store_id">
                <div class="form-group pull-right">
                    <button type="submit" class="btn btn-success saveGroup">Save</button>
                </div>
                {!! Form::close() !!}
            </div>
        </div>
                            ...
                            <tbody id="product-wrapper">
                                @foreach($storeGroup as $product)
                                    <tr>
                                        ...
                                        {{--<td>
                                                <a href="{{url('store/'.$store_id.'/admin/editProductGroups/'.$product->id)}}" title="Edit">
                                                    <i class="fa fa-edit fa-2x"></i>
                                                </a>

                                                <a href="#" title="Delete" data-toggle="confirmation"
                                                   data-href="{{url('store/'.$store_id.'/admin/deleteProductGroups/'.$product->id)}}">
...
@section('scripts')
    ...
    <script>
        ...
        jQuery(document).ready(function(e){
            jQuery('#productGroup').validate({
                errorElement : 'span',
                rules : {
                    'category_id' : {required:true},
                    'name'         : {required:true,maxlength:25},
                    'code'         : {required:true,maxlength:4 ,alphanum:true},
        ...
        jQuery(document).on('click','.saveGroup',function(e){
            jQuery('#productGroup').submit();
        });
        ...
        $(document).on("click", "#categories", function (e) {
            ...
            jQuery.ajax({
                url: '{{ url("store/$store_id/admin/productGroupFilter") }}',
                type: "Post",
