    ...
    @foreach($cartProducts as $brand_id => $products)
      ...
      <div class="col-md-12">
        <div class="row">
          <div class="cart-box">
            <div class="cart-title-box">
              <div class="seller"><?php
                ...
                ?>
                Seller: <a href="{{url('store/'.$brand->username)}}"
                           title="Click to got to store of '{{$brand->displayname}}'"> {{$brand->displayname}}</a>
              </div>
              ...
              <div class="product-added-box">
                ...
                <div class="col-md-4">
                  <div class="product-name">
                    ...
                    <?php $master_attribute_1 = ''; $master_attribute_2 = ''; $package_id = ''; $productKeeping = ''; ?>
                    @if(isset($product['master_attribute_1']))
                      <?php $productKeeping = getProductKeeping($product['product_id'], $product['master_attribute_1'], $product['master_attribute_2']); if(!isset($productKeeping->id)){continue;}?>
                      <div class="cs">
                    ...
                    @if(isset($product['master_attribute_2_label']))
                    <div class="cs">
            ...
            ">
                ...
                <div class="col-md-2">
                  <?php
                  if(!isset($productKeeping->price)){continue;}
                  ...
                  ?>
                  <div class="price row"><div style="color: #666666; font-size: 16px; margin-bottom: 10px; text-decoration: line-through;">{{format_currency($productKeeping->price)}} <sub>/ piece</sub></div>{{format_currency($price)}} <sub>/ piece</sub></div>
                  <?php
                ...
                <div class="col-md-1">
                  <a class="del-product delete-product"
                     href="{{url('store/cart/delete-product/by-url/'.$product['product_id'])}}">Delete</a>
                </div>
              </div>
            @endforeach
            <div class="col-md-4 col-md-offset-8 t-amount">
              ...
              @if($countCartProducts > 0)
                <div class="buy-this">
                  <a class="btn-proceed btn btn-default pull-right" id="btn-proceed"
                     href="{{url('store/'.$brand->username.'/shipping/address/'.\Vinkla\Hashids\Facades\Hashids::encode($brand_id))}}">Buy
                    from this seller</a>

                  <!--<button type="submit" class="btn btn-default">Buy from this seller</button>-->
                </div>
              @endif
            </div>
          ...
          @if($totalSellers > 0)
            <div class="buy-this">
              <a class="btn-proceed btn btn-success pull-right" id="btn-proceed"
                 href="{{url('store/'.$brand->username.'/shipping/address/buy-all')}}">Buy
                All</a>
            </div>
          @endif
          @else
