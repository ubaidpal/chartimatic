            ...
            @if(!$pending_withdrawals->isEmpty())
                <div class="dispute-wrapper">
                        ...
                        <div class="dispute-row">
                            ...
                            <div class="detail bb">
                                <div class="tn">
                                    ...
                                    <b>to {{$pending->method}} Account</b></div>
                                <div>Requested Date: &nbsp; {{$pending->created_at}}</div>
                                @if($pending->status == 'pending')
                                    <a class="greyBtn mt10 cancel cancel_request"
                                       href="{{url('store/cancelWithdrawalRequest/'.$pending->id)}}">Cancel</a>
                                @endif
                            </div>
                        </div>
                    @endforeach
                    ...
                    <div class="dispute-row">
                        ...
                        <div class="detail bb">Your withdrawals will be processed within <b>(7 - 14 business days)</b>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div class="dispute-wrapper" style="@if(count($errors) < 1) display: none; @endif" id="request_container">
                <form method="post" action="{{url('store/sendWithdrawalRequest')}}" id="requestForm"
                    ...
                    <div class="field-item dispute-row">
                        ...
                        <div class="detail bb">
                            <label>Available Balance<input checked type="radio" name="payment_type"
                                                           value="full"></label><br><br>
                            <label id="">Other Amount - $
                                <input type="radio" name="payment_type" value="partial">
                                <input disabled="disabled" type="text" name="amount" class="full_refund"
                                       placeholder="Amount">
                            </label>
                            @if(count($errors) > 0)
                    ...
                    <div class="dispute-row">
                        ...
                        <div class="detail bb">
                            <b id="fee_amount">${{format_currency($fee_amount)}}</b> ({{$fee_percentage}}% of the total
                            withdrawal amount)
                        </div>
                    </div>
                    <div class="dispute-row">
                        <div class="title mW">&nbsp;</div>
                        <div class="detail bb">You are about to send <b
                                    id="myAmount">${{format_currency(($available_balance - $pending_amount)- $fee_amount)}}</b>
    ...
    <div class="modal-box cart" id="cancel_request" style="display: none;">
        ...
        <div class="modal-body">
            <div class="edit-photo-poup">
                <p class="mt10" style="width: 400px;height: 30px;line-height: normal">Are you sure to cancel the payment
                    request?</p><br>
