...
class UsersRepository extends Repository implements RepositoryInterface
{
    ...
    public function find($id) {
        return $this->users->findOrNew($id);
    }
    ...
    public function get_user($user_id) {
        return User::whereId($user_id)->orWhere('username', $user_id)->first();
    }
    ...
    public function post_settings($user_id, $data) {
        ...
        $setting = Usersetting::whereSetting($item)->whereCategory($category)->whereUserId($user_id)->first();
        ...
        if (count($setting) > 0) {
            ...
            if ($item == 'DNT_DISPLY_ME_IN_SRCH') {
                $userCHangingSearchSetting = User::find($user_id);
