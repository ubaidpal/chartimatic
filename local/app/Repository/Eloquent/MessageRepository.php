...
class MessageRepository extends Repository
{
    ...
    public function get_conversations($user_id, $convId = FALSE) {
     ...
     * @return array
     */
    public function messages_detail($data) {
        ...
        foreach ($data as $row) {
            ...
            if(!is_null($row[ 'file_id' ]) && !empty($row[ 'file_id' ])) {
    ...
    public function addMessageToConversation($conv_id, $user_id, $data) {
        ...
        foreach ($usersInConv as $userInConv) {
            ...
            if($userInConv == $user_id) {
                ...
                if(env('PUSH_ENABLED', FALSE)) {
                    ...
                    $pushData[ "title" ]                     = $sender->name . " sent you a message";
                    $pushData[ "data" ][ "conversation_id" ] = $message->conv_id;
                    $pushData[ "data" ][ "message_body" ]    = $message->content;
                    $pushData[ "data" ][ "module" ]          = "messaging";
        ...
        return ['senderId' => $user_id, 'convUsersIds' => $usersInConv, 'content' => $data[ 'body' ], 'convId' => $conv_id, 'messageId' => $message->id,];
    }
    ...
    public function get_attachment_thumb($file_id) {
        return $thumb = Attachment::whereType('attachment_thumb')->whereParentFileId($file_id)->first();
    }
    ...
    public function parseUnreadMessages($unread) {
        $collection = collect($unread);
        $keyed      = $collection->keyBy('conv_id');
        return $keyed->all();
    }
    ...
    public function saveOtherMessage($data) {
        $value          = $data->file('attachment');
        $file[ 'data' ] = [];
        if(!empty($value)) {
        ...
        $this->save_message($data, $this->user_id);

        $data = $data->except(['middleware', '_token']);
